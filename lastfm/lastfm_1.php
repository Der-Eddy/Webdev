...
$played = false;
ImageTTFText ($im, 14, 0, 0, 20, ImageColorAllocate($im, 0, 0, 0), "arialuni.ttf", "Last heard on iTunes: $played");
for ($i = 0; $i <= $tracks; $i++){
    if (date_timestamp_get($date) - 5000 < $played_time && $played != 1){
        ImageTTFText ($im, 14, 0, 0, ($i + 1) * 20, ImageColorAllocate($im, 255, 0, 0), "arialuni.ttf", $listingto[$i] . $played);
        $played = true;
    } else {
        ImageTTFText ($im, 14, 0, 0, ($i + 1) * 20, $color, "arialuni.ttf", $listingto[$i] . $played);
    }
}
